...
class OracleDBQueryBuilderTest extends PHPUnit_Framework_TestCase {
    ...
    {
        $builder = $this->getBuilder();
        $builder->getConnection()->shouldReceive('insert')->once()->with('insert all into users (email) values (?) into users (email) values (?) select 1 from dual;', array('foo','bar'))->andReturn(true);
